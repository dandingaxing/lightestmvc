...
class Controller {
    ...
    public function createUrl($url, $params=array()){
        $baseUrl = empty($_SERVER['SCRIPT_NAME']) ? $_SERVER['PHP_SELF'] : $_SERVER['SCRIPT_NAME'];
        $urlArr = explode('/', trim(trim($url), '/'));
        $mArr = array();
        if (count($urlArr)===1) {
            $mArr[$this->_m] = $this->_module;
            $mArr[$this->_c] = $this->_controller;
            $mArr[$this->_v] = $urlArr[0];
        }elseif (count($urlArr)===2) {
            $mArr[$this->_m] = $this->_module;
            $mArr[$this->_c] = $urlArr[0];
            $mArr[$this->_v] = $urlArr[1];
        }elseif (count($urlArr)===3) {
            $mArr[$this->_m] = $urlArr[0];
            $mArr[$this->_c] = $urlArr[1];
            $mArr[$this->_v] = $urlArr[2];
        }else{
            $mArr[$this->_m] = $urlArr[0];
            $mArr[$this->_c] = $urlArr[1];
            $mArr[$this->_v] = $urlArr[2];
        ...
        // print_r($mArr);
        return $baseUrl . '?' . http_build_query( array_merge($mArr, $params) );
    }
