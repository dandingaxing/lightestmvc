...
class mysqliConnect{
    ...
    public function __construct($params = array()){
        ...
        $this->_conn = mysqli_connect($this->host, $this->user, $this->password, $this->database);
        mysqli_query($this->_conn, "set names ".$this->charset);
    }
     ...
     * @param  [type] $type [ 返回的格式 MYSQLI_ASSOC(关联数组) MYSQLI_NUM(数字数组) MYSQLI_BOTH(同时产生关联和数字数组)，默认为 MYSQLI_ASSOC(关联数组) ]
     * @return [type]       [ false查询失败，返回 Array 查询结果 ]
     */
    public function baseSelectOne($sql, $type=MYSQLI_ASSOC){
        $query=$this->query($sql);
     ...
     * @return [type]       [ false查询失败，返回 Array 查询结果 ]
     */
    public function baseSelect($sql, $type=MYSQLI_ASSOC){
        $query=$this->query($sql);
        if (empty($query)) {
            return false;
        }
    ...
    public function transaction($sqlArr = array()){
        ...
        $isCommit = true;
        // 开启事物
        mysqli_query($this->_conn, "BEGIN");    // 或者mysqli_query("START TRANSACTION");
        ...
        if($isCommit){
            // 事物提交
            mysqli_query($this->_conn, "COMMIT");
        }else{
            // 事物回滚
            mysqli_query($this->_conn, "ROLLBACK");
        }
        // 结束事物
        mysqli_query($this->_conn, "END");
        return $isCommit;
    }
