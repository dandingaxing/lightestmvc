...
class pdoConnect{
     ...
     *                           )
     */
    public function __construct( $config = array( 'dbhost'=>'127.0.0.1', 'dbport'=>'3306', 'dbuser'=>'root', 'dbpassword'=>'', 'dbname'=>'', 'dbcharset'=>'utf8', 'dboptions'=>array() ) ){
        $this->_DBHOST = empty($config['dbhost']) ? '127.0.0.1' : $config['dbhost'];
        $this->_DBPORT = empty($config['dbport']) ? 3306 : intval($config['dbport']);
        $this->_DBUSER = empty($config['dbuser']) ? 'root' : $config['dbuser'];
        $this->_DBPASSWORD = empty($config['dbpassword']) ? '' : $config['dbpassword'];
        $this->_DBNAME = empty($config['dbname']) ? '' : $config['dbname'];
        $this->_DBCHARSET = empty($config['dbcharset']) ? '' : $config['dbcharset'];
        if (is_array($config['dboptions']) && !empty($config['dboptions']) ) {
            $this->_DBOPTIONS = array_merge( $this->_DBOPTIONS, $config['dboptions'] );
        }
     ...
     * @return [type]                [ 查询返回多条数据结果 （二维数组/对象） ]
     */
    public function fetchAll($stmt, $paramkey='', $FetchMode = PDO::FETCH_ASSOC){
        if (empty($paramkey)) {
