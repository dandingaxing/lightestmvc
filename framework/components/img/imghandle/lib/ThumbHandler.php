...
/*
 使用方法:
    ...
    $t->setCutType(1);//这一句就OK了, 这句还可以默认省略, 使用默认裁切方式
    $t->setDstImg("tmp/new_test.jpg");
    $t->createImg(60,60); // 长宽可以自定义, 可以不是正方形
...
class ThumbHandler
{
    ...
    var $img_display_quality = 8;// 图片显示质量,默认为75
    ...
    // 文件类型定义,并指出了输出图片的函数,,imagejpeg, 用png流来处理jpeg图片更高清一点
    var $all_type = array(
        "jpg"  => array("output"=>"imagepng"),
        "gif"  => array("output"=>"imagegif"),
        "png"  => array("output"=>"imagepng"),
        "wbmp" => array("output"=>"image2wbmp"),
        "jpeg" => array("output"=>"imagepng"));
