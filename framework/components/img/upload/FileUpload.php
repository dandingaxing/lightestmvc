...
class FileUpload {
    ...
    public function clear(){
        $this->path = "./uploads"; //上传文件保存的路径
        $this->allowtype = array('jpg', 'gif', 'png'); //设置限制上传文件的类型
        $this->maxsize = 1000000; //限制文件上传大小（字节）
        $this->israndname = true; //设置是否随机重命名文件， false不随机
        $this->originName; //源文件名
        $this->tmpFileName; //临时文件名
        $this->fileType; //文件类型(文件后缀)
        $this->fileSize; //文件大小
        $this->newFileName; //新文件名
        $this->errorNum = 0; //错误号
        $this->errorMess = ""; //错误报告消息
    }
    ...
    public function setAllowtype($allowtype){
        return $this->set('allowtype', $allowtype);
    }
